...
\begin{exem}
Soient le langage des anneaux $\mathcal{L}_{ring} = \{0, 1, +, \times\}$ et son modèle $\mathbb{R}$, le corps des nombres réels.
Le sous-ensemble $[\pi, +\infty [{}$ est $\{\pi\}$-définissable via la formule :
\begin{align*}
...
\begin{exem}
Soient $\mathcal{L}_{ring}$ et son modèle $\mathbb{C}$, le corps des nombres complexes.
...
Soit $T$ une $\cL$-théorie. $T$ est satisfiable si et seulement si tout sous-ensemble fini de $T$ est satisfiable.
\end{theo*}
...
\[ \cF = \{ Y \subset I \mid X_0 \subset Y \} \]
...
tel que
\[ \cF = \{ X \subset I \mid x_0 \in X \} \]
Soit $I$ un ensemble infini, on définit
\[ \cF_{\text{ Fréchet }} = \{ X \subset I \mid I \setminus X \text{ est fini } \} \] C'est un filtre,
appelé filtre de Fréchet. Un ultrafiltre $\mathcal{U}$ est non principal si et seulement s'il contient le filtre de Fréchet.
\end{exem}
...
\begin{theo}[théorème de \L{}oś]\label{Los}
...
\noindent Proposons une seconde application du \hyperref[Los]{théorème de \L{}oś}.

\begin{prop}
Soit $\mathcal{U}$ un ultrafiltre non principal sur l'ensemble des nombres premiers $\mathbb{P}$.
Pour tout $p \in \mathbb{P}$, soit $\mathbb{F}^{\text{alg}}_p$ la cloture algébrique de $\mathbb{F}_p$ le corps à p éléments. 
L'ultraproduit $\mathcal{M} = \prod_{p \in \mathbb{P}} \mathbb{F}^{\text{alg}}_p / \mathcal{U}$ est un corps algébriquement clos
de caractéristique $0$.
\end{prop}

\begin{proof}
Nous introduisons la théorie des corps algébriquement clos de caractéristique nulle $ACF_0$ et les énoncés $\chi_p$ spécifiant la caractéristique $p$ dans la définition \ref{defACF}.
Par le théorème de \L{}oś, il est nécessaire et suffisant que pour tout $\phi \in~ACF_0$,
\[ \{p \in \mathbb{P} \mid \mathbb{F}^{\text{alg}}_p \models \phi \} \in \mathcal{U} \]
Soit $\phi$ un axiome de la théorie des corps algébriquement clos, alors
\[ \{p \in \mathbb{P} \mid \mathbb{F}^{\text{alg}}_p \models \phi \} = \mathbb{P} \]
car pour tout $p \in \mathbb{P}$, $\mathbb{F}^{\text{alg}}_p$ est un corps algébriquement clos. Or $\mathbb{P} \in \mathcal{U}$, 
donc par le théorème de \L{}oś, $\mathcal{M} \models \phi$ et $\mathcal{M}$ est un corps algébriquement clos. Soit $q \in \mathbb{P}$ fixé, on sait que 
pour tout $p \in \mathbb{P}, \mathbb{F}^{\text{alg}}_p$ est de caractéristique $p$, donc $\mathbb{F}^{\text{alg}}_p \models \neg \chi_q$
si et seulement si $p \ne q$. D'où 
\[ \{p \in \mathbb{P} \mid \mathbb{F}^{\text{alg}}_p \models \neg\chi_q \} = \mathbb{P} \setminus \{q\} \]
Or $\mathcal{U}$ est un ultrafiltre non principal sur $\mathbb{P}$ donc contient le filtre de Fréchet $\cF_{\text{ Fréchet }} = \{ X \subset \mathbb{P} \mid \mathbb{P}
\setminus X \text{ est fini } \}$.  Or $\{q\}$ est fini donc $\mathbb{P} \setminus \{q\} \in \cF_{\text{ Fréchet }} \subset \mathcal{U}$ donc $\cM \models \neg\chi_q$.
Ceci conclut que $\mathcal{M}$ est un corps algébriquement clos de caractéristique nulle.
\end{proof}
...
Ceci crée une nouvelle théorie, toujours finiment satisfiable (toute partie finie de la nouvelle théorie ne demandent qu'un nombre fini de constantes, lesquelles peuvent 
