...
\todo[inline]{rapide rappel produit cartesien}
\todo[inline]{finir filtre et exemple}
\todo[inline]{résultat sur structure de l'ultraproduit}

\subsection{Ultraproduits}

\noindent Soit $I$ un ensemble d'indices, $(A_i)_{i \in I}$ une famille d'ensembles non vide et $\mathcal{A} = \prod_{i \in I} A_i$ leur produit cartésien.
Soit $\mathcal{F}$ un filtre sur $I$. On définit une relation sur $\cA$ :
\begin{align*}
a \equiv_{\cF} b \text{ ssi } \{ i \in I \mid a(i) = b(i) \} \in \cF
\end{align*}
On vérifie que $\equiv_{\cF}$ est une relation d'équivalence. On note $a_{\cF}$ la classe de $a$ sur cette relation.

\noindent On introduit alors la définition suivante, en gardant les mêmes notations :
...
\begin{defiprop}
Soit $I$ un ensemble d'indices. Soient $(M_i)_{i \in I}$ une famille de $\cL$-structures et $\cM = \prod_{i \in I} A_i$ leur produit cartésien.
Soit $\mathcal{U}$ un ultrafiltre sur $I$. L'ultraproduit 
\end{defiprop}

\noindent Nous admettons le résultat pivot suivant :
...
car $T_0 \in V(\phi_1) \text{ via } \phi_1, T_0 \in V(\phi_2) \text{ via } \phi_2$ etc. Ainsi $\{V(\phi) \mid \phi \in T\}$ est contenue dans un filtre,
donc un ultrafiltre $\mathcal{U}$. On pose $\cM = \prod_{F \in I} \cM_F / \mathcal{U}$ l'ultraproduit des $\cM_F$. On montre que $\cM$ est un modèle de $T$.
...
\item Or $V(\phi) \in \mathcal{U}$ par construction, et $\mathcal{U}$ est en particulier un filtre, d'où $J_\phi \in \mathcal{U}$.
\end{enumerate}
Nous avons construit un modèle de $T$: $\cM \models T$.
\end{proof}

\begin{rema}
Le Théorème de \L{}oś et le quotient sur ultraproduit permettent ici une sorte de "passage à l'infini" par une propriété de "vrai presque partout" : 
c'est exactement le sens de $J_  {\phi} \in \mathcal{U}$. Intuitivement, cela rappelle le concept de mesure : on ne garde ici que ce qui est "vrai presque partout"
au sens de l'ultrafiltre, en laissant de côté les exceptions logiques locales.
\end{rema}
