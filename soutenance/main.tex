...
\begin{definition}[Élimination des quantificateurs]
  ...
  \begin{frame}{Le Dictionnaire Algèbre-Géométrie}
    ...
    \begin{block}{1. De l'Algèbre vers la Géométrie: l'ensemble des zéros communs}
        Soit $I \subset k[\bar{x}]$ un ensemble de polynômes. L'ensemble de leurs zéros communs est:
        \[ V(I) = \{ x \in k^n \mid \forall f \in I, f(x) = 0 \} \]
        
    \end{block}
    \pause
    \begin{block}{2. De la Géométrie vers l'Algèbre: l'idéal annulateur}
        Soit $V \subset k^n$ un ensemble de points. L'ensemble des polynômes s'annulant sur $V$ est:
        \[ I(V) = \{ f \in k[\bar{x}] \mid \forall x \in V, f(x) = 0 \} \]
    \end{block}
\end{frame}

% 
\begin{frame}{Une correspondance parfaite?}
    On vérifie facilement que $V(I(V)) = V$. \\
    \vspace{0.5cm}
