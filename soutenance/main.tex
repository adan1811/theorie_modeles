...
\begin{frame}{Langages}
    Un \textbf{langage} $\mathcal{L}$ est une collection de symboles divisée en trois types : les fonctions, les relations et les constantes.
    \pause
    \begin{example}
...
nous pouvons écrire des $\mathcal{L}$\textbf{-formules}.
\pause
\begin{example}
...
\begin{definition}
Soit $\mathcal{L}$ un langage. Une $\mathcal{L}$\textbf{-théorie} $\mathcal{T}$ est un ensemble de $\mathcal{L}$-formules.
Un \textbf{modèle} $\mathcal{M}$ de $\mathcal{T}$ est une structure mathématique où toutes les formules de $\mathcal{T}$ sont vraies dans $\mathcal{M}$,
et on note $\mathcal{M} \models \mathcal{T}$.
\end{definition}
\pause
\begin{example}
...
Rappel : $\phi_1 : \forall x,y,z [ x \times (y+z) = x \times y + x \times z ]$
\end{example}
\end{frame}
...
\begin{frame}{Théorie des corps algébriquement clos}
Soit $p \in \mathbb{P}$ et la $\mathcal{L}_{\text{ring}}$-formule $\chi_p : \underbrace{1 + \cdots + 1}_\text{p} = 0$.\\On définit :
\begin{enumerate}
...
\begin{frame}{Complétude}
    Une $\mathcal{L}$\textbf{-phrase} est une $\mathcal{L}$-formule sans variable libre.
...
\begin{frame}{Le Principe de Lefschetz}
    La complétude et la \textbf{compacité} permettent de transférer les phrases entre les caractéristiques:
    
    \begin{table}
        \centering
        \begin{tabular}{ll}
            \toprule
            \textbf{$\mathcal{L}_{\text{ring}}$-phrase $\phi$ vrai sur...} & \textbf{Équivaut à...} \\
            \midrule
...
\begin{frame}{Preuve : Étape 1 - Traduction de l'énoncé}
    On fixe $n$ et $d$ (degré maximum). L'application polynomiale
...
\begin{definition}[Élimination des quantificateurs]
  ...
  \begin{frame}{Le Dictionnaire Algèbre-Géométrie}
    Pour lier l'algèbre à la géométrie, on définit deux correspondances fondamentales entre les points de $k^n$ et les polynômes de $K[\overline{x}]$.
...
\begin{frame}{Une correspondance parfaite?}
    ...
    \vspace{0.5cm}
    \textbf{Question :} A-t-on $I(V(J)) = J$ pour tout idéal $J$?
    ...
    \begin{alertblock}{Contre-exemple}
        ...
        \pause
        \begin{enumerate}
            \item L'ensemble des zéros est: $V(J) = \{ 0 \}$. \pause
            \item L'idéal annulateur de ce point est: $J(\{0\}) = (X)$. \pause
            \item Conclusion: $(X) \neq (X^2)$.
        \end{enumerate}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Nullstellensatz}
    \begin{definition}[Radical d'un idéal]
        ...
        \[ \sqrt{J} = \{ f \in k[\bar{x}] \mid \exists m \ge 1, f^m \in J \} \]
    \end{definition}
...
\begin{frame}{Esquisse de la preuve}
    ...
    \[ K = (J, 1 - Yf) \subset k[\overline{x}, Y] \]
 
    \begin{enumerate}
        \item \textbf{Absence de zéros:} Si $x \in V(J)$, alors $f(x) = 0$, donc $1 - Y(0) = 1 \neq 0$. Ainsi, $V(K) = \emptyset$.

        \item \textbf{Nullstellensatz faible:} Par l'élimination des quantificateurs, $K$ n'est pas un idéal propre, donc $1 \in K$.
        
        \pause
        \item \textbf{Identité de Bézout:} Il existe des $A_i, B$ tels que 
        \[ 1 = \sum P_i A_i + (1-Yf)B \]
        \item \textbf{Évaluation:} En évaluant en $Y = 1/f$ et en chassant les dénominateurs, on obtient $f^m \in J$, d'où $f \in \sqrt{J}$.
    \end{enumerate}
\end{frame}

\begin{frame}{Annexe : Principe de Lefschetz. Partie 1.}
\begin{exampleblock}{Esquisse de la preuve}
Soit $\phi$ une $\mathcal{L}_{\text{ring}}$-phrase. Par hypothèse, $ACF_0 \models \phi$, donc il existe $\Sigma \subset ACF_0$ fini tel que $\Sigma \models \phi$,
sinon :
\pause
\begin{enumerate}
\item Pour tout $\Delta \subset ACF_0$ fini, $\Delta \not\models \phi$.
\pause
\item $\Delta \cup \{ \neg\phi \}$ est satisfiable.
\pause
\item $ACF_0 \cup \{ \neg\phi \}$ est finiment satisfiable donc satisfiable par \textbf{compacité}.
\pause
\item $ACF_0 \not\models \phi$.
\end{enumerate}
\end{exampleblock}
...
\begin{frame}{Annexe : Principe de Lefschetz. Partie 2.}
\begin{exampleblock}{Esquisse de la preuve}
$\Sigma$ est fini donc ne peut empêcher qu'un nombre fini de caractéristiques positives. Il existe $p \in \mathbb{P}$ suffisament grand
tel que $ACF_p \models \Sigma$, donc $ACF_p \models \phi$.
\end{exampleblock}
\end{frame}
\end{document}